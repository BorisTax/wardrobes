...
export function getCorpusSpecification(wardrobe: IWardrobe, profileType: ProfileType, coefList: Map<SpecificationItem, number>): Map<SpecificationItem, number> {
    ...
    spec.set(SpecificationItem.DVP, wardrobe.getDVP() * (coefList.get(SpecificationItem.DVP) || 1))
...
export function getWardrobe(data: WardrobeData): IWardrobe {
    switch (data.wardKind) {
        case WARDROBE_KIND.STANDART:
            return new StandartWardrobe(data)
        default:
            return new StandartWardrobe(data)
    }
}