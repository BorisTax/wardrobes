...
export async function updatePriceList({ name, caption, coef, price, code, id, markup }: SpecificationData) {
  const priceService = new PriceService(priceServiceProvider)
  ...
  if (!(priceList as SpecificationData[]).find(m => m.name === name)) return noExistData(messages.PRICELIST_NAME_NO_EXIST)
