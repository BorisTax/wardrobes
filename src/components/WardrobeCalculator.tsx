        ...
        <div style={{ display: "flex", justifyContent: "center" }}>
            <div>
                <PropertyGrid style={{ padding: "0.5em", border: "1px solid" }}>
                    <ComboBox title="Тип шкафа:" value={WardrobeTypesCaptions.get(wardType) as string} items={WardrobeTypes} onChange={(_, value) => { setState(prev => ({ ...prev, wardType: value as WARDROBE_TYPE })) }} />
                ...
                <PropertyGrid style={{ padding: "0.5em", border: "1px solid" }}>
                    ...
                    {fasades.dsp.count > 0 && fasades.dsp.names.map((m, i) => <ComboBox key={"dsp" + i} title={`${i + 1}`} items={dsp10List} value={fasades.dsp.names[i]} onChange={(_, value) => { fasades.dsp.names[i] = value; setState(prev => ({ ...prev })) }} />)}
                    ...
                    {fasades.mirror.count > 0 && fasades.mirror.names.map((m, i) => <ComboBox key={"mirror" + i} title={`${i + 1}`} items={mirrorList} value={fasades.mirror.names[i]} onChange={(_, value) => { fasades.mirror.names[i] = value; setState(prev => ({ ...prev })) }} />)}
                    ...
                    {fasades.fmp.count > 0 && fasades.fmp.names.map((m, i) => <ComboBox key={"fmp" + i} title={`${i + 1}`} items={fmpList} value={fasades.fmp.names[i]} onChange={(_, value) => { fasades.fmp.names[i] = value; setState(prev => ({ ...prev })) }} />)}
                    ...
                    {fasades.sand.count > 0 && fasades.sand.names.map((m, i) => <ComboBox key={"sand" + i} title={`${i + 1}`} items={sandList} value={fasades.sand.names[i]} onChange={(_, value) => { fasades.sand.names[i] = value; setState(prev => ({ ...prev })) }} />)}
                    ...
                    {fasades.lacobel.count > 0 && fasades.lacobel.names.map((m, i) => <ComboBox key={"lacobel" + i} title={`${i + 1}`} items={lacobelList} value={fasades.lacobel.names[i]} onChange={(_, value) => { fasades.lacobel.names[i] = value; setState(prev => ({ ...prev })) }} />)}
                    ...
                    {fasades.lacobelGlass.count > 0 && fasades.lacobelGlass.names.map((m, i) => <ComboBox key={"lacobelGlass" + i} title={`${i + 1}`} items={lacobelGlassList} value={fasades.lacobelGlass.names[i]} onChange={(_, value) => { fasades.lacobelGlass.names[i] = value; setState(prev => ({ ...prev })) }} />)}
                </PropertyGrid>
                <PropertyGrid style={{ padding: "0.5em", border: "1px solid" }}>
                ...
                <PropertyGrid style={{ padding: "0.5em", border: "1px solid" }}>
                    ...
                    <ComboBox title="Тип консоли:" value={ConsoleTypesCaptions.get(extComplect.console.type) || ""} items={ConsoleTypes} onChange={(_, value: string) => { setState(prev => ({ ...prev, extComplect: { ...prev.extComplect, console: { ...prev.extComplect.console, type: value as CONSOLE_TYPE } } })) }} />
                </PropertyGrid>
                <PropertyGrid style={{ padding: "0.5em", border: "1px solid" }}>
