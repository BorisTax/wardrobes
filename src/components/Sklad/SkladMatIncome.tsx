...
import { matSkladColorsAtom, matSkladIncomeAtom, matSkladOutcomeAtom, matSkladThickAtom } from "../../atoms/skladMat"
...
export default function SkladMatIncome({ income }: { income: boolean }) {
    ...
    const { filterId, filterMinLength, filterMaxLength, filterMinWidth, filterMaxWidth, filterMinDate, filterMaxDate, filterThick, setFilter } = useFilter(income, minDate)
    const list = listFull.filter(s => (s?.id === filterId || filterId === 0) && (matColors.get(s.id)?.thickId === filterThick || filterThick === 0) && s?.length >= filterMinLength && s?.length <= filterMaxLength && s?.width >= filterMinWidth && s?.width <= filterMaxWidth && s?.date >= filterMinDate && s?.date <= filterMaxDate)
    ...
    const header: TableDataHeader[] = [{ caption: "Дата" }, { caption: "Материал" }, { caption: "Толщина" }, { caption: "Длина" }, { caption: "Ширина" }, { caption: "Кол-во" }, { caption: "Пользователь" }]
    const contents = list.map(ss => ({ key: ss.id, data: [getDateFormat(ss.date), matColors.get(ss.id)?.name, matThickness.get(matColors.get(ss.id)?.thickId || 0), ss.length, ss.width, ss.count, ss.user] }))
    useEffect(() => {
    ...
    return <div className="sklad-container">
        ...
        <GroupBox caption="Фильтр">
            ...
            <br />
            <ComboBox value={filterId} items={listDistinctId} displayValue={v => {
            ...
            <input type="button" value="Сбросить" onClick={() => setFilter({ filterId: 0, filterMinLength: 0, filterMaxLength: 4000, filterMinWidth: 0, filterMaxWidth: 4000, filterMinDate: minDate, filterMaxDate: Date.now(), filterThick: 0 })} />
        </GroupBox>
        <hr />
...
function useFilter(income: boolean, minDate: number) {
    const [{ filterId: filterId1, filterMinLength: filterMinLength1, filterMaxLength: filterMaxLength1, filterMinWidth: filterMinWidth1, filterMaxWidth: filterMaxWidth1, filterMinDate: filterMinDate1, filterMaxDate: filterMaxDate1, filterThick: filterThick1 }, setFilter1] = useState({ filterId: 0, filterMinLength: 0, filterMaxLength: 4000, filterMinWidth: 0, filterMaxWidth: 4000, filterMinDate: minDate, filterMaxDate: Date.now(), filterThick: 0 })
    const [{ filterId: filterId2, filterMinLength: filterMinLength2, filterMaxLength: filterMaxLength2, filterMinWidth: filterMinWidth2, filterMaxWidth: filterMaxWidth2, filterMinDate: filterMinDate2, filterMaxDate: filterMaxDate2, filterThick: filterThick2 }, setFilter2] = useState({ filterId: 0, filterMinLength: 0, filterMaxLength: 4000, filterMinWidth: 0, filterMaxWidth: 4000, filterMinDate: minDate, filterMaxDate: Date.now(), filterThick: 0 })    
    return {
