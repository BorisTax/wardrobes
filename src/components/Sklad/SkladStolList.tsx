...
import { addStolAtom, clearStolAtom, deleteStolAtom, loadStolColorsAtom, loadStolIncomeAtom, loadStolSkladAtom, stolColorsAtom, stolIncomeAtom, stolOutcomeAtom, stolSkladAtom } from "../../atoms/skladStol"
...
export default function SkladStolList() {
    ...
    const editItems: EditDataItem[] = [
        { title: "Столешница:", value: stolSklad[selected.row]?.id, displayValue: (value) => stolColors.get(value as number) || "", inputType: InputType.LIST, list: [...stolColors.keys()], checkValue: (value) => ({ success: !!value, message: "Выберите материал" })  },
        { title: "Длина:", value: stolSklad[selected.row]?.length,nullValue: 0,  inputType: InputType.TEXT, propertyType: PropertyType.INTEGER_POSITIVE_NUMBER, checkValue: (value) => ({ success: value as number > 0, message: "Длина должна быть больше 0" }) },
        { title: "Кол-во:", value: stolSklad[selected.row]?.amount, nullValue: 0,  inputType: InputType.TEXT, propertyType: PropertyType.INTEGER_POSITIVE_NUMBER, checkValue: (value) => ({ success: value as number > 0, message: "Кол-во должно быть больше 0" }) }
    ]
    useEffect(() => {
        loadStolColors()
        loadStolSklad()
