...
import { Detail, SpecificationResultItem, WARDROBE_KIND } from "../../../types/wardrobe"
...
export async function getCorpusSpecification(wardrobe: IWardrobe, data: WardrobeData, profileType: ProfileType, coefList: Map<SpecificationItem, number>): Promise<Map<SpecificationItem, SpecificationResultItem>> {
    ...
    const confirmat = (await getConfirmat(data)).data
    const minifix = (await getMinifix(data)).data
    const karton = (await getKarton(data)).data
    spec.set(SpecificationItem.DSP, (await getDSP(data)).data)
    spec.set(SpecificationItem.DVP, (await getDVP(data)).data)
    spec.set(SpecificationItem.Kromka2, (await getEdge2(data)).data)
    spec.set(SpecificationItem.Kromka05, (await getEdge05(data)).data)
    ...
    spec.set(SpecificationItem.Glue, (await getGlue(data)).data)
    spec.set(SpecificationItem.Planka, (await getDVPPlanka(data)).data)
    spec.set(SpecificationItem.Leg, (await getLegs(data)).data)
    spec.set(SpecificationItem.Karton, karton)
    spec.set(SpecificationItem.Skotch, { amount: karton.amount * 20 * (coefList.get(SpecificationItem.Skotch) || 1) })
    spec.set(SpecificationItem.Nails, (await getNails(data)).data)
